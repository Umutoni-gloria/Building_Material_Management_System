...
const EditOrder = () => {
  ...
  // Load order data and dependencies
  useEffect(() => {
    const loadData = () => {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedProducts = products.map(product => {
        const originalOrderItem = originalOrder.items.find(item => item.productId === product.id);
        const newOrderItem = selectedProducts.find(item => parseInt(item.productId) === product.id);
        
        let stockChange = 0;
        if (originalOrderItem) stockChange += originalOrderItem.quantity;
        if (newOrderItem) stockChange -= parseInt(newOrderItem.quantity);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Update order details and items</p>
      </div>
